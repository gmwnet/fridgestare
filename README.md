...
- **Group inventory by tag** — Toggle between flat list and tag-grouped view. Multi-tag items appear in each group. Click an item to edit its name or tags inline.
- **Self-upgrading** — `php upgrade.php` checks GitHub, backs up your site, and upgrades everything. Full-site zip snapshots saved before each upgrade in `_version_backups/`.
- **Cache busting** — JS assets load with version query params. On upgrade, open tabs auto-detect the change and hard-refresh to load the new UI.
- **Upgrade script at your fingertips** — The "Check for upgrades" link on the Settings page points to the latest release. Download `upgrade.php` from any release and run it.
